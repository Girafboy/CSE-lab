...
fuseserver_readlink(fuse_req_t req, fuse_ino_t ino) {
    ...
    if (yfs->readlink(ino, buf) != yfs_client::OK) {
        fuse_reply_err(req, ENOENT);
        return;
    }
...
fuseserver_symlink(fuse_req_t req, const char *link,
    ...
    yfs_client::inum id;
    yfs_client::status stat;
    struct fuse_entry_param e;

    stat = yfs->symlink(parent, name, link, id);
    if (stat != yfs_client::OK) {
        if (stat == yfs_client::EXIST) {
            fuse_reply_err(req, EEXIST);
        } else {
            fuse_reply_err(req, ENOENT);
        }
